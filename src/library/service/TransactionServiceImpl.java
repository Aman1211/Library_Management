	...
	public void renew(TransactionTO tt) {
		...
		try {
			...
			message.setContent("<b> Hello " + ut.getName()
					+ ",</b><br><h3>The following book/s have been renewed in your name</h3><br>" + bt.getTitle() + ","
					+ bt.getAuthor() + ",Duedate:- " + ddate + "<br><br> Regards,<br>Library Admin.", "text/html");
...
//			 tdao.markFine(tt.getTransactionId(), 0);
	...
	public void notifyreturn(String isbn, String uname) {
		BookTO b = bdao.Exists(isbn);
		UserTO u = udao.Exists(uname);
		Properties props = new Properties();
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (entry.getValue().equals(true)) {
					...
					sb.append(cnt + ". " + "<b>" + b.getTitle() + "</b>" + "," + b.getAuthor() + ",Duedate:"
							+ tt1.getDuedate()).append("<br>");
					cnt++;
				}
			}
			message.setContent("<b> Hello " + ut.getName()
					+ ",</b><br><h3>The following book/s have been issued in your name</h3><br>" + sb
					+ "<br><br> Regards,<br>Library Admin.", "text/html");
