...
public class ActionChangePasswordValidate implements Action {
	...
	public String perform(HttpServletRequest request, HttpServletResponse response) {
		...
		if (username.isEmpty() || password.isEmpty() || current_password.isEmpty() || confirm_password.isEmpty()) {
			...
			request.setAttribute("errMessage1", err); // If authenticateUser() function returnsother than SUCCESS string
		...
		if (password.equals(confirm_password) == false) {
			...
			if (m.matches() == false) {
				...
				request.setAttribute("errMessage1", err); // If authenticateUser() function returnsother than SUCCESS
															// string it will be sent to Login page again. Here the
															// error message returned from function has been stored in a
															// errMessage key.
