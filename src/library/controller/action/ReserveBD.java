...
public class ReserveBD {
	...
	*/
	...
	private static ReserveBean TO2Bean(ReserveTO bkt) {
		...
		/*BookBean book = new BookBean(bkt.getISBNBook().getISBN(), bkt.getISBNBook().getTitle(), 
				bkt.getISBNBook().getAuthor(), bkt.getISBNBook().getCategory(),bkt.getISBNBook().getQty(),bkt.getISBNBook().getImage(),bkt.getISBNBook().getRack());
		*/
		ReserveBean reserve = new ReserveBean(bkt.getId(),bkt.getUsername(), 
				bkt.getISBN());
		return reserve;
	}
