...
public class RequestBD {
	...
	private static RequestBean TO2Bean(RequestTO bkt) {
		RequestBean request = new RequestBean(bkt.getId(), bkt.getTitle(), bkt.getAuthor(), bkt.getCategory(),
				bkt.getImage(), bkt.getUsername(), bkt.getStatus());
		return request;
	}

	private static RequestTO Bean2TO(RequestBean bk) {
		RequestTO request = new RequestTO(bk.getTitle(), bk.getAuthor(), bk.getCategory(), bk.getImage(),
				bk.getUsername());
		return request;
	}
